import { delCategory, listCategory } from '@/api/content/category';
...
const Article = () => {
  ...
  const getList = async (current, size) => {
    ...
    setLoading(true);
    const res = await listCategory(data);
    if (res.code === 200) {
  ...
  const handleDelete = async (row) => {
    const ids = row.id;
    const res = await delCategory(ids);
    if (res.code === 200) {
    ...
    <div className='viewBox'>
      <Form form={form}>
        <Row className={styles.btnBox}>
          <Col span={6}>
            <Form.Item name='name' label='分类名' className='searchItem'>
      ...
      <Table
        ...
        rowKey='id'
      />
      {isOpen && <AddCategory isOpen={isOpen} changeOpen={changeOpen} setIsOpen={setIsOpen} activeData={activeData} />}
    </div>
  );
};
