import { delMenu, listMenu } from '@/api/menu';
...
const Article = () => {
  ...
  const getList = async (current, size) => {
    ...
    setLoading(true);
    const res = await listMenu(data);
    if (res.code === 200) {
      const treeList = handleTree(res.data, 'id');
      setTableData(treeList);
  ...
  return (
    <div className='viewBox'>
      ...
      <Table
        ...
        rowKey='id'
      />
      {isOpen && <AddMenu isOpen={isOpen} changeOpen={changeOpen} setIsOpen={setIsOpen} activeData={activeData} />}
    </div>
  );
};
